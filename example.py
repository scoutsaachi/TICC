...
def runTest(mode, inputName, outputName, clusters, beta, gamma, motifReq, oldAssignmentsName):
	...
	solver = TICCSolver(window_size=3, number_of_clusters=clusters, lambda_parameter=5e-3, beta=beta, threshold=2e-5, gamma=gamma, input_file=inputName, num_proc=30, maxMotifs=50, motifReq=motifReq, maxIters=20)
	old_assign=None
	usemotif = False
	if mode == 1:
		print("using motif")
	...
	print(motifs)
	if mode  == 1:
		fname = "%s_clust%s_beta%s_gamma%s_req%s.out" % (outputName, clusters, beta, gamma, motifReq)
	else:
